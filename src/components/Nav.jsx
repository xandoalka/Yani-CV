    ...
    <nav className="lg:h-28 flex items-center fixed top-0 w-full bg-white z-10">
      <div className="lg:w-11/12 py-6 px-5 lg:py-0 lg:px-0 w-full lg:m-auto flex justify-between">
        ...
        <button onClick={handleButtonClick} className="flex z-10 lg:hidden flex-col justify-center w-10 cursor-pointer">
          <div className={`bg-secondary rounded-lg h-1 mb-1.5 transition-all duration-300 ${isChecked ? 'w-1/2 transform rotate-45 translate-x-1 translate-y-1 bg-black' : 'w-1/2'}`}></div>
          <div className={`bg-secondary rounded-lg h-1 transition-all duration-300 w-full ${isChecked ? 'w-11/12 transform -rotate-45 bg-black' : 'w-full'} `}></div>
          <div className={`bg-secondary rounded-lg h-1 mt-1.5 transition-all duration-300 ${isChecked ? 'w-1/2 transform rotate-45 translate-x-4 -translate-y-1 bg-black' : 'w-3/4'}`}></div>
        </button>
